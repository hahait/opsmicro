    ...
    <div class="user_info">
      <div class="userinfo-img-container">
        ...
        <p class="userinfo-img-text">
          <strong style="color: #3c3a3a">{{ userInfo.name }} ({{ userInfo.cn_name }})</strong>
        </p>
        <el-row>
...
import { editUser, editUserPasswd } from '@/api/accounts/user'
...
export default {
  ...
  methods: {
    getMySelfData: function() {
      getUserInfo().then(
        (res) => {
          ...
          const { group, ...userdata } = res.results
          // 下面的代码作用：遍历 group 中的 permission(是个数组), 将所有 permission 中所有元素汇总到 plist 中，然后去重
          var plist = []
          group.forEach(g => {
            plist = _.concat(plist, g.permissions)
          })
          // 删除每个 permission 对象中的 id 属性，否则不能去重（因为 id 不一样）
          plist.forEach(p => {
            delete p.id
          })
          plist = _.uniqWith(plist, _.isEqual)
          // 去重结束
          this.userGroups = group
          this.userPermissions = plist
          const user = [
            { name: '角色', value: userdata.role },
            { name: '邮箱', value: userdata.email },
            { name: '手机', value: userdata.phone },
            { name: '管理员', value: userdata.is_admin },
            { name: '状态', value: userdata.is_active },
            { name: '最近登录日期', value: userdata.last_login },
            { name: '最近修改日期', value: userdata.UpdatedAt }
          ]
          console.log('当前获取的 user: ', user)
    ...
    // 提交修改个人信息
    commitChangeMyselfInfo: function(data) {
      editUser(data).then(
        () => {
          this.$message({
    ...
    // 提交修改密码
    commitChangeMyselfPassword: function(data) {
      editUserPasswd(data).then(
        () => {
          const ss = this.$store
          this.$message({
