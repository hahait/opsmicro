  ...
  <div class="group-list">
    ...
    <el-table
      ...
      <el-table-column
        ...
        align="center">
        <template slot-scope="scope">
          <el-button v-if="scope.row.user" type="text" size="mini" plain @click="viewGroupUser(scope.row)">
            查看 ( {{ scope.row.user.length }} ) 人
          </el-button>
          <el-button v-else type="text" size="mini" plain @click="viewGroupUser(scope.row)">
            查看 ( 0 ) 人
          </el-button>
      ...
      <el-table-column
        ...
        align="center">
        <template slot-scope="scope">
          <el-button v-if="scope.row.permissions" type="text" size="mini" plain @click="viewGroupPermisssion(scope.row)">
            查看 ( {{ scope.row.permissions.length }} ) 个
          </el-button>
          <el-button v-else type="text" size="mini" plain @click="viewGroupPermisssion(scope.row)">
            查看 ( 0 ) 个
          </el-button>
...
export default {
  ...
  methods: {
    ...
    changeGroupUser: function(row) {
      const group_users_list = row.user
      var uid_list = []
      for (var u in group_users_list) {
        uid_list.push(group_users_list[u]['ID'])
      }
      this.row = row
      this.users = uid_list
    ...
    commitChange: function() {
      const data = { id: this.row.ID, users: this.users }
      console.log('此时的 data: ', data)
    ...
    groupDelete: function(row) {
      this.$confirm(`<p style="margin:20px">你确定将永久<b style="color:#ff0000;">删除该用户组: \< ${row.name} \> </b> 吗?<p>`, '嘿，知道自己在干嘛吗', {
