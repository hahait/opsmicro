...
<script>
import { getUserList, addUser, editUser, deleteUser, editUserStatus, editUserAdmin, editUserPasswd, editUserGroups } from '@/api/accounts/user'
...
export default {
  ...
  methods: {
    ...
    changeAdminStatus: function(row) {
      editUserAdmin({ id: row.ID, is_admin: row.is_admin }).then(
        () => {
          this.$message({
    ...
    changeStatus: function(row) {
      editUserStatus({ id: row.ID, is_active: row.is_active }).then(
        () => {
          this.$message({
    ...
    commitChangeUserObj: function(form_data) {
      ...
      editUser(form_data).then(
        () => {
          this.$message({
            message: 'Good , 用户信息修改成功',
            type: 'success'
          })
    ...
    // 修改密码提交
    commitChangeUserPasspwdObj: function(form_data) {
      console.log('在 user/index 中获得的 form_data: ', form_data)
      editUserPasswd(form_data).then(
        () => {
          this.$message({
            message: 'Good , 用户密码修改成功',
            type: 'success'
          })
        ...
        (error) => {
          ...
          this.$message({
            message: 'Oh , 修改用户密码失败,错误信息: ' + errmsg,
            type: 'error'
          })
        }
      )
    ...
    changeUserGroupsBtn: function(row) {
      var gid = []
      for (var g in row.group) {
        gid.push(row.group[g]['ID'])
      }
    ...
    // 修改用户组表单提交
    commitChangeUserGroups: function(data) {
      // const { id, ...data } = groups
      editUserGroups(data).then(
        () => {
          this.$message({
